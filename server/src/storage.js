...
class StorageManager {
    ...
    thresholdPercent = 80; // 80% full triggers cleanup
    targetPercent = 70; // Clean up until 70%
    constructor() {
                ...
                for (const camDir of dirsToCheck) {
                    ...
                    if (isNewStructure) {
                        const dateDirs = fs.readdirSync(camDir).filter(d => {
                            try {
                                return fs.statSync(path.join(camDir, d)).isDirectory();
                            }
                            catch {
                                return false;
                            }
    ...
    async purgeOldestRecordings(targetPercent) {
